...
gulp.task("debug", ["scripts"], function () {
    return cordovaBuild.buildProject(platformsToBuild, ["--debug"]);
});
...
gulp.task("debug-android", ["scripts"], function (callback) {
    cordovaBuild.setupCordova().done(function (cordova) {
        cordova.run({ platforms: ["android"], options: ["--debug", "--device"] }, callback);
    });
});

gulp.task("run-android", ["scripts"], function (callback) {
    cordovaBuild.setupCordova().done(function (cordova) {
        cordova.run({ platforms: ["android"], options: ["--release", "--device"] }, callback);
    });
});
